...
world_peer::~world_peer(void)
{
	mgr_handler = nullptr;
}

uint16_t world_peer::get_type()
{
	return (uint16_t)server_protocols::e_st_world;
}

void world_peer::on_peer_event(e_peer_event eps)
{
	__ENTER_FUNCTION
...
void world_peer::heartbeat( double elapsed )
{
	__ENTER_FUNCTION
...
void world_peer::set_mgr_handler(server_manager_handler* _handler)
{
	mgr_handler = _handler;
}

int world_peer::send_msg_to_client(uint32_t sessionid, uint16_t packet_id, boost::shared_ptr<google::protobuf::Message> msg)
{
...
int world_peer::send_msg_to_client(const std::list<uint32_t>& sessionids, uint16_t packet_id, boost::shared_ptr<google::protobuf::Message> msg)
{
