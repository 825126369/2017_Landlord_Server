...
void server_peer::regedit_to_monitor()
{
	...
	sendmsg->set_server_port(gate_server::instance().get_serverid());
	sendmsg->set_server_type(server_protocols::e_server_type::e_st_gate);
	send_msg( sendmsg);
	//std::cout << "服务器 注册: "<<get_remote_type() << std::endl;
}

void server_peer::regedit_to_server()
{
	auto sendmsg = PACKET_CREATE(packet_server_connect, e_mst_server_connect);
	sendmsg->set_server_id(gate_server::instance().get_serverid());
	sendmsg->set_server_type(server_protocols::e_server_type::e_st_gate);
	send_msg(sendmsg);
	//std::cout << "服务器 注册: "<<get_remote_type() << std::endl;
}