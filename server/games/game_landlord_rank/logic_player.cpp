...
void logic_player::heartbeat(double elapsed)
{
	...
	if (is_robot())//机器人AI
	{
		...
		else if (player_state == e_player_game_state::e_player_game_state_playhanding)
		{
			...
			{
				game_landlord_rank_protocol::card_Info cards = m_table->get_rob_playhand(this);
...
//出牌
e_server_error_code logic_player::playhand(const game_landlord_rank_protocol::card_Info& cards)
{
