#include "StdAfx.h"
#include "logic_playhandAI_1.h"
#include <stdlib.h>
#include <time.h>
#include <stdio.h>
#define WM_ON_SENDCARD  (WM_USER+103)
logic_playhandAI_1::logic_playhandAI_1(void)
	{
		m_sendArray.max = -1;
		m_noSendNum =2;
	}
//洗牌
void logic_playhandAI_1::Shuffle(int *P)
{
	int randValue;
	int m;
	int n = 0;
...
CARDTYPE logic_playhandAI_1::JudgeCardsType(vector<SENDCARDS>& sendCard)//判断牌型(参数:出牌的数组，最大值)
{
	...
	//判断飞机是否连着
bool  logic_playhandAI_1::JudeLink(int * temp)
{
...
CARDSTYLE temp;
int arrayTemp[15];
void logic_playhandAI_1::ClassifyCards(vector<SCARD>&  player)//对牌进行分类
{
	m_bomb.clear();
	m_double.clear();
	m_doubleLink.clear();
...
void logic_playhandAI_1::JudeFly(vector<CARDSTYLE> &Three, vector<CARDSTYLE> &plane, bool is)//判断飞机
{
	if (Three.size() >= 2)
	{
		int index = 0;
...
void logic_playhandAI_1::JudeDoubleLink(vector<CARDSTYLE> &Double, vector<CARDSTYLE> &linkDouble, bool is)//判断连队
{
	if (Double.size() >= 3)
	{
		int index = 0;
...
void logic_playhandAI_1::JudeLink()//判断连牌
{
	int index = 0;
	int j = 0;
	for (int i = 0; i < 11; i++)
...
//从一个向量中删除元素
void logic_playhandAI_1::DeleteElement(vector<CARDSTYLE> &src, vector<CARDSTYLE>& des)
{
	if (des.size() != 0)
	{
...
void logic_playhandAI_1::SplitCards()//拆牌
{
...
//拆牌，说有能组成的牌型添加进行遍历
//炸弹，飞机，连对，3条，对子
void logic_playhandAI_1::ErgodicCard(bool bosb, bool plane, bool linDouble, bool three, bool Double)
{
	RegainCard();//恢复原始牌
	zong.s_bosb.clear();
	zong.s_double.clear();
...
void logic_playhandAI_1::RegainCard()//恢复牌到最开始的牌型
{
//炸弹
   for (int i=0;i<15;i++)
   {
...
bool logic_playhandAI_1::sendPlane(vector<SCARD>& player, vector<SENDCARDS>& send, int  ID, int x, int y)
{
	int  Customunsigned;
	if (ID == PLAYER_C)
	{
...
bool logic_playhandAI_1::DeleteComputerCard(vector<SCARD>& player, CARDTYPE type, vector<SENDCARDS>& send, int  ID, int x, int y)//删除电脑的牌
{
	int  Customunsigned;
	if (ID == PLAYER_C)
	{
...
CARDTYPE logic_playhandAI_1::SendStely()//计算当前出牌的类型
{
...
bool logic_playhandAI_1::PlayerSendCards(int  num, int player, bool down, vector<SENDCARDS>& send, vector<SCARD>& playerSend)//玩家出牌出牌
{
	int x, y;
	if (player == PLAYER_C)
	{
