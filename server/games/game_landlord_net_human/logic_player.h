...
public:
	...
	e_server_error_code rob_enter_room(int roomId, int deskId);
	e_server_error_code enter_room(int roomId, std::string password);			//进入房间
	e_server_error_code enter_room(int roomId);
	...
	void get_invite_player_list(packetl2c_join_player_list_result* sendmsg);
	e_server_error_code invite_player(int playerId);
	void add_invite_me_player(int playerId);
	e_server_error_code or_agree_join_room(int roomId, bool orAgree);
	e_server_error_code or_prepare(bool orPrepare);
	int get_wait_time();

	e_server_error_code robLandlord(int);// 抢地主
	e_server_error_code playhand(const game_landlord_net_human_protocol::card_Info& cards);// 出牌
	e_server_error_code SetOrHang(bool orHang);//托管
	void do_protobuf_send_SceneInfo();
	void do_protobuf_notice_invite_room_list(logic_player* player, int roomId);
	bool is_inTable() { return m_table != nullptr; }

	bool get_orPrepare() { return orPrepare; }
	bool set_orPrepare(bool orPrepare) { return this->orPrepare=orPrepare; }
	e_player_state get_game_state();
	e_player_game_state get_player_game_state() { return player_state; }
	void set_player_game_state(e_player_game_state state);
...
private:
	...
	std::map<int32_t,invitation_info> invitePlayerMap;
	...
	double rob_match_cd;
	double rob_match_max_cd;
	double rob_robLandlord_cd;
	double rob_robLandlord_max_cd;
	double rob_playhand_cd;
	double rob_playhand_max_cd;
};