...
public:
	...
	game_landlord_net_humanmachine_protocol::card_Info get_rob_playhand(logic_player* player) { return m_cardManager->get_rob_playhand(player); }
	bool judge_rob_orLandlord(logic_player* player) { return m_cardManager->judge_rob_orRobLandlord(player); }

	e_server_error_code playhand(logic_player* player, const game_landlord_net_humanmachine_protocol::card_Info& cards);
	e_server_error_code rob_Landlord(logic_player* player,int);

	void do_protobuf_notice_playhand(logic_player* player, const game_landlord_net_humanmachine_protocol::card_Info& cards);//通知 出牌
	void get_Scene_Info(logic_player* player, game_landlord_net_humanmachine_protocol::packetl2c_get_room_scene_info_result* sendmsg);//获取场景信息
private:
...
private:
	...
	const Landlord_NetHumanMachine_RoomCFGData* m_cfg;
};