	...
	,lastPlayhand_Id(0)
{
	...
	m_cfg = Landlord_NetHumanMachine_RoomCFG::GetSingleton()->GetData(m_room->get_room_id());
	...
	else if (gameState == e_game_state::e_game_state_playhand)//玩家出牌
	{
		if (playhand_cdTime > 15.0)
		{
			for (auto& var : playerMap)
			{
				...
				{
					game_landlord_net_humanmachine_protocol::card_Info cards = this->get_rob_playhand(var.second.get());
...
e_server_error_code logic_table::playhand(logic_player* player, const game_landlord_net_humanmachine_protocol::card_Info& cards)
{
	...
	if (cards.cards_size() > 0)
	{
		...
		if (lastPlayhand_Id != cards.deskid() && lastPlayhand_Id>0)
		{
			...
			{
				std::string str;
				for (int i = 0; i < lastPlayhand.size(); i++)
				{
					str += std::to_string(lastPlayhand[i]) + " ";
				}
				SLOG_CRITICAL << "上家出牌: " << str;
				str="";
				for (int i = 0; i < cards.cards_size(); i++)
				{
					str += std::to_string(cards.cards().Get(i)) + " ";
				}
				SLOG_CRITICAL << "出牌信息有误"<<str;
				return e_server_error_code::e_error_code_failed;
			}
		}
...
void logic_table::do_protobuf_notice_start_game()//开始游戏
{
	...
	int score = Landlord_NetHumanMachine_RoomCFG::GetSingleton()->GetData(m_room->get_room_id())->mScore;
...
void logic_table::do_protobuf_notice_playhand(logic_player* player, const game_landlord_net_humanmachine_protocol::card_Info& cards)//通知  所有人 出牌信息
{
	auto sendmsg = PACKET_CREATE(packetl2c_notice_playhand, e_mst_l2c_notice_playhand);
...
void logic_table::get_Scene_Info(logic_player* player, game_landlord_net_humanmachine_protocol::packetl2c_get_room_scene_info_result* sendmsg)
{
