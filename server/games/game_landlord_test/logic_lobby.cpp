...
{
	Landlord_NetHumanMachine_RoomCFG::GetSingleton()->Load();
}
...
{
	const boost::unordered_map<int, Landlord_NetHumanMachine_RoomCFGData>& list = Landlord_NetHumanMachine_RoomCFG::GetSingleton()->GetMapData();
...
int logic_lobby::enter_room(uint32_t pid, uint16_t rid)
{
	...
	if(room == roomMap.end())
		return e_error_code_failed;;										//返回失败
