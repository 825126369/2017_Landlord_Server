...
static const int MAX_ROBOT_COUNT = 1000;

logic_room::logic_room(const Landlord_NetHumanMachine_RoomCFGData* cfg, logic_lobby* _lobby):robCount(0),createRob_time(0)
{
	m_cfg = cfg;
	m_lobby = _lobby;
...
e_server_error_code logic_room::enter_room(LPlayerPtr player)
{
	...
	int gold = Landlord_NetHumanMachine_RoomCFG::GetSingleton()->GetData(get_room_id())->mGoldCondition;
	if (player->get_gold() < gold)
	{
		return e_error_code_not_enough_money;
	}
...
const Landlord_NetHumanMachine_RoomCFGData* logic_room::get_room_cfg() const
{
	return m_cfg;
}
